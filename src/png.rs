...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        ensure!(index.is_some(), "Failed to find chunk with chunktype");
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        ...
        value.read_exact(&mut buffer)?;
            ...
            buffer == Self::STANDARD_HEADER,
            "Png does not start with standard header"
        );

        let mut chunks = Vec::new();
        while let Ok(chunk) = Chunk::try_from(value) {
            value = &value[chunk.as_bytes().len()..];
            chunks.push(chunk);
        }
